import { createClient } from "@/lib/supabase/server"
import { StatsClient } from "@/components/stats/stats-client"

export const revalidate = 0

export default async function StatsPage() {
  const supabase = await createClient()

  // Get all matches
  const { data: matches } = await supabase
    .from("matches")
    .select("*")

  // Get all match player stats
  const { data: matchStats } = await supabase
    .from("match_player_stats")
    .select(`
      *,
      hero:heroes(*),
      player:players(*)
    `)

  // Get tournaments
  const { data: tournaments } = await supabase
    .from("tournaments")
    .select("*")

  // Get finances
  const { data: finances } = await supabase
    .from("finances")
    .select("*")

  // Calculate team stats
  const totalMatches = matches?.length || 0
  const wins = matches?.filter(m => m.is_win).length || 0
  const losses = totalMatches - wins
  const winrate = totalMatches > 0 ? Math.round((wins / totalMatches) * 100) : 0

  const totalTournaments = tournaments?.length || 0
  const completedTournaments = tournaments?.filter(t => t.status === "completed").length || 0
  const totalPrize = finances?.filter(f => f.type === "income").reduce((sum, f) => sum + Number(f.amount), 0) || 0

  // Calculate hero stats (most picked, most banned, best winrate)
  const heroPickMap = new Map<string, { picks: number; wins: number; name: string; role: string }>()
  matchStats?.forEach(stat => {
    if (stat.hero_id && stat.hero) {
      const existing = heroPickMap.get(stat.hero_id) || { picks: 0, wins: 0, name: stat.hero.name, role: stat.hero.role }
      existing.picks++
      if (stat.is_win) existing.wins++
      heroPickMap.set(stat.hero_id, existing)
    }
  })

  const heroStats = Array.from(heroPickMap.entries()).map(([id, data]) => ({
    id,
    name: data.name,
    role: data.role,
    picks: data.picks,
    wins: data.wins,
    winrate: data.picks > 0 ? Math.round((data.wins / data.picks) * 100) : 0,
  }))

  const mostPickedHeroes = [...heroStats].sort((a, b) => b.picks - a.picks).slice(0, 5)
  const bestWinrateHeroes = [...heroStats].filter(h => h.picks >= 3).sort((a, b) => b.winrate - a.winrate).slice(0, 5)

  // Calculate player stats
  const playerStatsMap = new Map<number, { 
    id: number
    nickname: string
    matches: number
    wins: number
    mvps: number
    kills: number
    deaths: number
    assists: number
  }>()

  matchStats?.forEach(stat => {
    if (stat.player) {
      const existing = playerStatsMap.get(stat.player_id) || {
        id: stat.player_id,
        nickname: stat.player.nickname,
        matches: 0,
        wins: 0,
        mvps: 0,
        kills: 0,
        deaths: 0,
        assists: 0,
      }
      existing.matches++
      if (stat.is_win) existing.wins++
      if (stat.is_mvp) existing.mvps++
      existing.kills += stat.kills || 0
      existing.deaths += stat.deaths || 0
      existing.assists += stat.assists || 0
      playerStatsMap.set(stat.player_id, existing)
    }
  })

  const playerStats = Array.from(playerStatsMap.values()).map(p => ({
    ...p,
    winrate: p.matches > 0 ? Math.round((p.wins / p.matches) * 100) : 0,
    kda: p.deaths > 0 ? ((p.kills + p.assists) / p.deaths).toFixed(2) : (p.kills + p.assists).toFixed(2),
  }))

  const topMvpPlayers = [...playerStats].sort((a, b) => b.mvps - a.mvps).slice(0, 5)
  const topWinratePlayers = [...playerStats].filter(p => p.matches >= 3).sort((a, b) => b.winrate - a.winrate).slice(0, 5)

  return (
    <StatsClient
      teamStats={{
        total_matches: totalMatches,
        wins,
        losses,
        winrate,
        total_tournaments: totalTournaments,
        completed_tournaments: completedTournaments,
        total_prize: totalPrize,
      }}
      mostPickedHeroes={mostPickedHeroes}
      bestWinrateHeroes={bestWinrateHeroes}
      topMvpPlayers={topMvpPlayers}
      topWinratePlayers={topWinratePlayers}
    />
  )
}
